...
public class PokerHand {
	...
	public static PokerHand of(final String representation){
		return new PokerHand(FunctionalList.of(representation.split(" ")).map(new Function<String, Card>() {
			...
			@Override
			public Card apply(final String input) {
	...
	private Category determineCategory(final FunctionalList<Category> categories){
	...
	public enum Category{
		...
		ROYAL_FLUSH("Royal Flush",new Function<PokerHand,Boolean>(){
			@Override
			public Boolean apply(final PokerHand input) {
		...
		private Category(final String rep, final Function<PokerHand,Boolean> categoryEvaluator){
			this.rep = rep;
			this.categoryEvaluator = categoryEvaluator;
		}
		
		public boolean evaluate(final PokerHand hand){ return categoryEvaluator.apply(hand);}
